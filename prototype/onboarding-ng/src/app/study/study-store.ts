...
export class StudyStore {
  ...
  /**
   * What class actually covered, by day. This is the app's spine: coaching
   * owns the sequence, so the order these are logged in becomes the order the
   * plan works through — nobody hand-sorts a hundred chapters.
   */
  readonly classLog = persisted<{ dateKey: string; chapterIds: string[] }[]>('class-log', []);

  logClass(dateKey: string, chapterIds: string[]): void {
    const rest = this.classLog().filter((e) => e.dateKey !== dateKey);
    const next = chapterIds.length === 0 ? rest : [...rest, { dateKey, chapterIds }];
    this.classLog.set(next.sort((a, b) => a.dateKey.localeCompare(b.dateKey)));
  }

  classOn(dateKey: string): string[] {
    return this.classLog().find((e) => e.dateKey === dateKey)?.chapterIds ?? [];
  }

  /** Every chapter class has reached, oldest first — the real sequence. */
  readonly classCovered = computed(() => {
    const out: string[] = [];
    for (const entry of this.classLog()) {
      for (const id of entry.chapterIds) if (!out.includes(id)) out.push(id);
    }
    return out;
  });

  /**
   * A test result, per subject. Not a diary — the only reason to hold it is
   * that a bad subject rewrites the queue: what you got wrong comes back
   * sooner, whatever the schedule said.
   */
  readonly testResults = persisted<
    { dateKey: string; label: string; scores: { subjectId: string; correct: number; total: number }[] }[]
  >('test-results', []);

  logTest(dateKey: string, label: string, scores: { subjectId: string; correct: number; total: number }[]): void {
    const rest = this.testResults().filter((t) => t.dateKey !== dateKey);
    this.testResults.set([...rest, { dateKey, label, scores }].sort((a, b) => a.dateKey.localeCompare(b.dateKey)));
    this.applyTestToQueue(scores);
  }

  readonly lastTest = computed(() => this.testResults().at(-1) ?? null);

  /**
   * Pull revision forward for the subjects that went badly. Anything under
   * half marks comes back inside three days rather than waiting its turn.
   */
  private applyTestToQueue(scores: { subjectId: string; correct: number; total: number }[]): void {
    const weak = scores
    ...
    const soon = dateKey(addDays(startOfToday(), 2));
    for (const [id, stat] of next) {
      if (!weak.some((subjectId) => id.startsWith(subjectId + '.'))) continue;
      if ((stat.dueKey ?? '') <= soon) continue;
      next.set(id, { ...stat, dueKey: soon });
    }
    this.stats.set(next);
  }

  /** The chapters class covered today, which the evening should lead with. */
  readonly classToday = computed(() => this.classOn(dateKey(startOfToday())));
