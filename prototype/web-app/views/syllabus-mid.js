/* Midway variant of views/syllabus.js — same logic and tick state, sizes
   halfway between A (dense) and B (roomy). Separate file so both others
   stay untouched. */
...
export function mountSyllabusMid(root, { navigate }) {
  ...
  function nodeHTML(node, key, hours, depth) {
    ...
    return `
      <div class="node d${depth}${open ? ' open' : ''}" data-key="${key}">
        <div class="row">
          ${hasKids
