/* ============================================================
   data.js — exams, syllabus trees, scheduler, state.
   Every screen imports this. Do not duplicate this data anywhere.
...
/* Syllabus trees keyed by exam id.
   Shape: [{ n: section, q: questions in the paper, t: [[topic, hours], ...] }]
...
const DEFAULT_STATE = {
  ...
  we:7,
  place:'',
  done:{},        // { "sectionIndex_topicIndex": true }
  missedDays:0,   // used by screen 09
};
...
function saveState(s){
  try{ localStorage.setItem('plan', JSON.stringify(s)); }catch(e){}
}
function resetState(){
...
/* ---------- syllabus helpers ---------- */
function syllabusFor(examId){ return SYLLABUS[examId] || SYLLABUS.cgl; }
function examFor(examId){ return EXAMS.find(e=>e.id===examId) || EXAMS[0]; }

function rawHours(examId){
  let h=0;
...
function needHours(s){ return Math.round(rawHours(s.exam) * REVISION_MULTIPLIER); }

function availableHours(s){
...
function daysUntil(dateStr){
  return Math.max(1, Math.round((new Date(dateStr) - new Date().setHours(0,0,0,0)) / 864e5));
}
function isWeekend(d){ return [0,6].includes((d||new Date()).getDay()); }
function todayBudget(s){ return isWeekend() ? s.we : s.wd; }
